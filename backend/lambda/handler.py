...
def get_important_memories(db, user_id, limit=10):
    """Get high-importance long-term memories — baseline fallback"""
    try:
...
def semantic_search_memories(db, user_id, query, limit=8):
    """
    Phase H: Semantic memory search using Groq as a relevance ranker.
    Fetches top 40 memories by importance, then uses Groq to pick the
    most semantically relevant ones for the current query.
    Falls back to importance-ranked memories if Groq fails.
    """
    try:
        ...
        try:
            ...
            docs = db.collection(f'users/{user_id}/memories').limit(40).stream()
        ...
        # Use Groq to rank by semantic relevance to current query
        mem_list = '\n'.join([
            f"{i+1}. [{m['category']}] {m['text']} ({m['date']})"
            for i, m in enumerate(candidates)
        ])

        rank_prompt = f"""Given this user query, rank which memories are most relevant.
...
Return ONLY a JSON array of the {limit} most relevant memory numbers (1-indexed), ordered by relevance.
Example: [3, 7, 1, 12, 5, 8, 2, 15]
Return ONLY the JSON array, nothing else."""

        response = ask_groq([{'role': 'user', 'content': rank_prompt}], max_tokens=100)
        clean = response.strip().replace('```json', '').replace('```', '').strip()

        if '[' in clean:
            indices = json.loads(clean[clean.index('['):clean.rindex(']')+1])
            ranked = []
            for idx in indices:
    ...
    # Fallback: return top by importance
    return candidates[:limit]

def search_all_content(db, user_id, query, limit=5):
    """
    Phase H: Search across research reports, content scripts, strategy reports
    using Groq semantic ranking. Returns mixed results with source type.
    """
    results = []
    try:
        # Research reports
        try:
            rdocs = db.collection(f'users/{user_id}/research_reports') \
                      .order_by('timestamp', direction='DESCENDING').limit(20).stream()
        except Exception:
            rdocs = db.collection(f'users/{user_id}/research_reports').limit(20).stream()
        for d in rdocs:
            data = d.to_dict()
            results.append({
                'type': 'research',
                'id': d.id,
                'title': data.get('topic', data.get('query', 'Research')),
                'preview': str(data.get('report', ''))[:200],
                'date': data.get('date', ''),
            })
    except Exception as e:
    ...
    try:
        # Content scripts
        try:
            cdocs = db.collection(f'users/{user_id}/content_scripts') \
                      .order_by('created_at', direction='DESCENDING').limit(20).stream()
        except Exception:
            cdocs = db.collection(f'users/{user_id}/content_scripts').limit(20).stream()
        for d in cdocs:
            data = d.to_dict()
            results.append({
                'type': 'content',
                'id': d.id,
                'title': f"{data.get('content_type', 'Script')}: {data.get('topic', d.id)}",
                'preview': str(data.get('script', data.get('content', '')))[:200],
                'date': data.get('date', ''),
            })
    except Exception as e:
    ...
    try:
        # Strategy reports
        sdocs = db.collection(f'users/{user_id}/strategy_reports').limit(6).stream()
        for d in sdocs:
            data = d.to_dict()
            results.append({
                'type': 'strategy',
                'id': d.id,
                'title': f"Strategy: {data.get('month_name', d.id)}",
                'preview': str(data.get('report', ''))[:200],
                'date': data.get('month', ''),
            })
    except Exception as e:
    ...
    # Semantic rank via Groq
    try:
        items_list = '\n'.join([
            f"{i+1}. [{r['type']}] {r['title']} — {r['preview'][:100]}"
            for i, r in enumerate(results)
        ])
        rank_prompt = f"""Rank these documents by relevance to the query.
...
Return ONLY a JSON array of the {min(limit, len(results))} most relevant document numbers (1-indexed).
Example: [2, 5, 1]
Return ONLY the JSON array."""

        response = ask_groq([{'role': 'user', 'content': rank_prompt}], max_tokens=80)
        clean = response.strip().replace('```json', '').replace('```', '').strip()
        if '[' in clean:
            indices = json.loads(clean[clean.index('['):clean.rindex(']')+1])
            ranked = []
        ...
        print(f"Content semantic rank error: {e}")
...
def lambda_handler(event, context):
    try:
        ...
        # ── Phase H: Semantic search across all content ──
        if action == 'semantic_search':
            query = body.get('query', user_message)
            search_type = body.get('search_type', 'all')  # all | memories | content
            try:
                if search_type == 'memories':
                    results = semantic_search_memories(db, user_id, query, limit=10)
                    return _response({'results': results, 'type': 'memories', 'query': query, 'status': 'ok'})
                elif search_type == 'content':
                    results = search_all_content(db, user_id, query, limit=10)
                    return _response({'results': results, 'type': 'content', 'query': query, 'status': 'ok'})
                else:
                    mem_results = semantic_search_memories(db, user_id, query, limit=5)
                    content_results = search_all_content(db, user_id, query, limit=5)
                    return _response({
        ...
        important_memories = semantic_search_memories(db, user_id, user_message, limit=8)
