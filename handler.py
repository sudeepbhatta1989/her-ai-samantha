    ...
    if intent == 'research':
        ...
        try:
            ...
            if plan_doc.exists:
                ...
                # No plan saved yet — generate one
                ...
                for slot in (new_plan.get('morning', []) + new_plan.get('afternoon', []) + new_plan.get('evening', [])):
                    ...
                    if t and task:
                        slot_summary.append(f"{t} — {task}")
                lines = '\n'.join(slot_summary[:10])
                reply = f"No plan was saved for tomorrow yet, so I created one for you ({day_name}):\n\n{lines}\n\n🎯 Top priority: {new_plan.get('top_priority', '')}\n\nTell me if anything needs to change — WFH, meetings, cancelled commute etc."
...
def generate_plan_for_date(db, user_id, target_date):
    ...
    # Pull last 3 conversation messages as context
    recent_convs = get_recent_conversations(db, user_id, limit=3)
    recent_context = ""
    if recent_convs:
        recent_context = "Recent context from Sudeep:\n" + "\n".join([
            f"- {c.get('userMessage', '')[:120]}"
            for c in recent_convs[-3:] if c.get('userMessage')
        ])
...
def lambda_handler(event, context):
    try:
        ...
        if action == 'get_plan':
            # Accept optional date param — validate it matches today for safety
            req_date = body.get('date', '')
            today_str = datetime.date.today().isoformat()
            if req_date and req_date != today_str:
                # Flutter sent a date that isn't today — return empty to force fresh generation
                return _response({'plan': None, 'status': 'date_mismatch', 'today': today_str})
            plan = get_todays_plan(db, user_id)
            has_slots = (
                isinstance(plan.get('morning'), list) and len(plan.get('morning', [])) > 0 or
                isinstance(plan.get('afternoon'), list) and len(plan.get('afternoon', [])) > 0 or
            ...
            return _response({'plan': plan if has_slots else None, 'status': 'ok'})
